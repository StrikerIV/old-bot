...
exports.run = async (client, message, args) => {
    ...
    if (checkforUser) {
        ...
        if (checkforUser.deleted) {
            return message.reply({ embeds: [BotError(client, "The supplied `GuildMember` is `deleted`.")] });
        }
        ...
        return message.reply({ embeds: [BotSuccess(client, `${checkforUser} has been warned.\n\nReason: ${reasonForWarn}`, { footer: `Case #${insertQuery.data.insertId}` })] })
        ...
        if (!userToWarn) {
            return message.reply({ embeds: [BotError(client, "The supplied `GuildMember` is `invalid`.")] })
        }
        ...
        return message.reply({ embeds: [BotSuccess(client, `${userToWarn} has been warned.\n\nReason: ${warnReason.data}`, { footer: `Case #${insertQuery.data.insertId}` })] })
    }
    ...
    if (!fetchedWarnData) {
        return message.reply({ embeds: [BotError(client, "This warn does not exist.")] })
    }
    if (fetchedWarnData.guild_id != message.guild.id) {
        return message.reply({ embeds: [BotError(client, "This warn does not pertain to your guild.")] })
    }
    ...
    if (warnModifier === "edit") {
        ...
        if (fetchedWarnData.moderator_id != message.author.id) {
            if (!message.member.hasPermission("ADMINISTRATOR")) {
                return message.reply({ embeds: [BotError(client, `You cannot edit this warn.`)] })
            }
        }
        ...
        if (choiceToDo === null) {
            ...
            return message.reply({ embeds: [BotError(client, `This command has timed out.`)] })
        } else if (choiceToDo === "Reason") {
            // edit the reason to user supplied message
            await message.channel.send({ embeds: [BotOngoing("What would you like to set the reason to?", { color: "BLUE", footer: "You have 30 seconds." })] })
            ...
            return message.reply({ embeds: [BotSuccess(client, `Warn \`#${warnReasonOrUser.data}\` has been set to -\n\n${newReason.content}`)] })
            ...
            return message.reply({ embeds: [BotSuccess(client, `Warn \`#${warnReasonOrUser.data}\` has been closed.`)] })
            ...
            return message.reply({ embeds: [BotSuccess(client, `Warn \`#${warnReasonOrUser.data}\` has been reopened.`)] })
        }
        ...
        return message.reply({
            embeds: [BotSuccess(client, `Warn \`#${warnReasonOrUser.data}\` has been removed from <@${fetchedWarnData.user_id}>.`, { footer: `\nTip: You can edit a warn using "!warn edit <case#>"` })]
        })
        ...
        return message.reply({ embeds: [viewWarnEmbed] })
    }
}
